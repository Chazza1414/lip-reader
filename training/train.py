...
def train(run_name, start_epoch, stop_epoch, img_c, img_w, img_h, frames_n, minibatch_size, dataset_path):
    ...
    lipreader_generator = Generator(minibatch_size=minibatch_size, start_epoch=start_epoch, dataset_path=dataset_path).build()
    ...
    evaluation_loss, evaluation_accuracy = lipreader.model.evaluate(x=LockedIterator(lipreader_generator.next_evaluate()), 
                                                                    verbose=1, callbacks=[tensorboard], metrics=[accuracy])
